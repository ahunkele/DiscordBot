...
@bot.command(pass_context=True)
async def commands():
    await bot.say("set target to mute with the :D mute @member command")
    await bot.say("then run the :D mute_matthew command to start the muting/defaning")
    await bot.say(":D stop to stop the process. May take a few seconds to go through.")
